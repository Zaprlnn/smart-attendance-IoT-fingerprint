...
import type { AttendanceMethod, AttendanceStatus, Course, Session } from "@/lib/types"
...
const EXCEL_COLUMNS: ExcelColumn<RekapRow>[] = [
  {
    header: "Tanggal",
    width: 14,
    value: (row) => format(new Date(row.tanggal), "d MMM yyyy", { locale: id }),
  },
  ...
  { header: "Status", width: 12, value: (row) => ATTENDANCE_STATUS_LABEL[row.status], align: "center" },
  {
    header: "Metode",
    width: 14,
    value: (row) => (row.method === "fingerprint" ? "Fingerprint" : "Manual"),
    align: "center",
  },
]

function calculateLateMinutes(timestampStr: string, tanggalStr: string, jamMulaiStr: string): number {
  try {
    ...
    return Math.round((checkIn.getTime() - classStart.getTime()) / 60_000)
  } catch {
    return 0
  }
}
...
export default function RekapPresensiPage() {
  ...
  const [apiRows, setApiRows] = useState<RekapApiRow[]>([])
  const [sessionOptions, setSessionOptions] = useState<Session[]>([])

  const [courseFilter, setCourseFilter] = useState("all")
  const [sessionFilter, setSessionFilter] = useState("all")
  const [statusFilter, setStatusFilter] = useState("all")
  const [cutoffPeriod, setCutoffPeriod] = useState(30)
  ...
  useEffect(() => {
    if (!lecturer) return
    apiFetch<{ data: Course[] }>(`/mata-kuliah?dosenId=${lecturer.id}`).then((res) => setCourses(res.data))
  }, [lecturer])

  useEffect(() => {
    if (!lecturer) return
    const qs = courseFilter === "all" ? "" : `?mataKuliahId=${courseFilter}`
    apiFetch<{ data: RekapApiRow[] }>(`/presensi/rekap${qs}`).then((res) => setApiRows(res.data))
  }, [lecturer, courseFilter])

  useEffect(() => {
    if (courseFilter === "all") return
    apiFetch<{ data: Session[] }>(`/mata-kuliah/${courseFilter}/sesi`).then((res) =>
      setSessionOptions(res.data.filter((s) => s.status !== "akan-datang"))
    )
  }, [courseFilter])

  const courseById = useMemo(() => new Map(courses.map((c) => [c.id, c])), [courses])
  const visibleSessionOptions = useMemo(
    () => (courseFilter === "all" ? [] : sessionOptions),
    [courseFilter, sessionOptions]
  )
  const sessionById = useMemo(
    () => new Map(visibleSessionOptions.map((s) => [s.id, s])),
    [visibleSessionOptions]
  )

  const rows = useMemo<RekapRow[]>(() => {
    return apiRows
      .filter((r) => sessionFilter === "all" || r.sesiId === sessionFilter)
      .map((r) => {
        let status = r.status
        if (status === "hadir") {
          const late = calculateLateMinutes(r.timestamp, r.tanggal, r.jamMulai)
          if (late > cutoffPeriod) status = "alpha"
        }
        return {
          key: r.id,
          tanggal: r.tanggal,
          ...
          courseNama: r.courseNama,
          status,
          waktu: format(new Date(r.timestamp), "HH:mm"),
          method: r.method,
        }
      })
  }, [apiRows, sessionFilter, cutoffPeriod])

  const filteredRows = useMemo(() => {
    const query = search.trim().toLowerCase()
    return rows
      .filter((row) => statusFilter === "all" || row.status === statusFilter)
      .filter((row) => !query || row.nama.toLowerCase().includes(query) || row.nim.includes(query))
      .sort((a, b) => {
  ...
  const attendancePct = totalRows === 0 ? 0 : Math.round((summary.hadir / totalRows) * 100)
  ...
  const hasActiveFilter =
    courseFilter !== "all" || sessionFilter !== "all" || statusFilter !== "all" || search.trim() !== ""
  ...
  async function handleExport() {
    ...
    setIsExporting(true)
    try {
      const courseLabel = courseFilter === "all" ? "Semua Mata Kuliah" : courseById.get(courseFilter)?.nama ?? "Mata Kuliah"
    ...
    <>
      ...
      <div className="rounded-xl border border-border bg-card p-5 shadow-soft mb-6 space-y-4">
        <div className="flex items-center gap-2 text-primary">
          <span className="font-semibold text-sm">Kebijakan Presensi</span>
        </div>
        <p className="text-xs text-muted-foreground">
          Geser batas toleransi keterlambatan di bawah ini. Status mahasiswa dan persentase kehadiran akan dihitung ulang berdasarkan jam mulai mata kuliah.
        </p>
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-[10px] text-muted-foreground">
              Mahasiswa yang terlambat &gt; {cutoffPeriod} menit akan otomatis ditandai sebagai Alpha.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {STATUS_OPTIONS.map((status) => (
          <div key={status} className="rounded-xl border border-border bg-card p-4 shadow-soft hover-lift">
            <p className="text-sm text-muted-foreground">{ATTENDANCE_STATUS_LABEL[status]}</p>
            <p className="font-display text-2xl font-semibold">{summary[status]}</p>
          </div>
        ))}
        <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 shadow-soft hover-lift">
          ...
          <p className="font-display text-2xl font-semibold text-primary">{attendancePct}%</p>
        </div>
      </div>
        ...
        <Select value={courseFilter} onValueChange={(v) => updateCourseFilter(v ?? "all")}>
          <SelectTrigger className="w-52">
            <SelectValue placeholder="Mata kuliah">
              {(value: string | null) =>
                !value || value === "all" ? "Semua Mata Kuliah" : courseById.get(value)?.nama ?? "Mata kuliah"
              }
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
          ...
          <SelectContent>
            ...
            {visibleSessionOptions.map((session) => (
              <SelectItem key={session.id} value={session.id}>
                Pertemuan {session.pertemuanKe} • {format(new Date(session.tanggal), "d MMM yyyy", { locale: id })}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        ...
        >
          <SelectTrigger className="w-40">
            <SelectValue placeholder="Status">
              {(value: string | null) =>
                !value || value === "all" ? "Semua Status" : ATTENDANCE_STATUS_LABEL[value as AttendanceStatus]
              }
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
              ...
              <TableBody>
                {pageRows.map((row) => (
                  <TableRow key={row.key}>
                    <TableCell>{format(new Date(row.tanggal), "d MMM yyyy", { locale: id })}</TableCell>
                    <TableCell className="text-muted-foreground">{row.waktu}</TableCell>
                    <TableCell className="text-muted-foreground">{row.nim}</TableCell>
