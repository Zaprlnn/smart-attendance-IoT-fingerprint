...
import { serializeMataKuliah } from "../lib/serializers.js"
import { deriveSesiStatus, toJakartaIsoDate, dayNameOf } from "../lib/sesi-status.js"
...
function canAccessDosen(auth: { sub: string; role: string }, dosenId: string) {
  return auth.role === "lecturer" && auth.sub === dosenId
}

// GET /dosen/:id/dashboard — ringkasan utk dashboard dosen: stat cards, jadwal hari ini,
...
dosenRouter.get("/:id/dashboard", requireAuth("lecturer"), async (req, res) => {
  if (!canAccessDosen(req.auth!, req.params.id)) {
